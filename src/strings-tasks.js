...
function sumOfCodes(str) {
  if (typeof str === 'string' || str instanceof String)
    return Array.from(str, (x) => x.charCodeAt(0)).reduce((a, b) => a + b, 0);
  return 0;
}
...
function isPalindrome(str) {
  const s = str.toLowerCase().match(/[a-z]/gi);
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((long, curr) => {
...
function invertCase(str) {
  return Array.from(str, (x) =>
    x.charCodeAt(0) === x.toLowerCase().charCodeAt(0)
      ? x.toUpperCase()
      : x.toLowerCase()
  ).join('');
}
...
function unbracketTag(str) {
  ...
  const withoutBracket = str.slice(startIndex, endIndex);
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    if ((upperCase && shifted > 90) || (!upperCase && shifted > 122)) {
      shifted -= 26;
    }
